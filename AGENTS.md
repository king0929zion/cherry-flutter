...
### 2025-11-02 设置与输入区优化 ✅
- 复刻设置首页分组卡片样式，接入头像入口并对接个人资料页。
- 新增「个人资料」页面，可编辑昵称与头像并本地持久化。
- 调整聊天输入框高度与工具尺寸，移除 API Key 限制提示。

### 2025-11-02 输入与设置像素对齐加强 ✅
- 聊天输入框重塑：压缩高度、优化阴影与安全区，工具按钮和 @ 胶囊按原版比例重排。
- Mentions pill 支持展示已选助手的 emoji / 数量，底部选择器增加多选确认流程。
- 设置首页卡片统一圆角、阴影与间距常量，头像渐变与裁切恢复 Cherry Studio 的细节表现。
- 最近主题列表改为轻量行样式，激活态绿色高亮与时间排版对齐原项目。
